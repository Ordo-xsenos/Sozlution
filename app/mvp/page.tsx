    ...
    <main className={`mvp-shell ${isDark ? 'mvp-dark' : 'mvp-light'} relative min-h-screen overflow-hidden ${isDark ? 'bg-[#050b18] text-[#dbeafe]' : 'bg-[#f4f8fd] text-[#1e2b3d]'}`}>
      ...
      <div className="relative z-10 flex min-h-screen flex-col lg:flex-row">
        ...
        <section className="relative flex-1 p-3 pt-14 sm:p-6 lg:p-8">
          ...
          {view === 'learn' && <div className="mx-auto max-w-3xl"><h2 className="text-4xl font-black sm:text-5xl">{learnStep === 1 ? t.step1 : learnStep === 2 ? t.step2 : t.step3}</h2><div className="mt-3 flex gap-2"><div className={`h-2 flex-1 rounded-full ${learnStep >= 1 ? 'bg-[#4f9df1]' : 'bg-[#e7edf5]'}`} /><div className={`h-2 flex-1 rounded-full ${learnStep >= 2 ? 'bg-[#4f9df1]' : 'bg-[#e7edf5]'}`} /><div className={`h-2 flex-1 rounded-full ${learnStep >= 3 ? 'bg-[#4f9df1]' : 'bg-[#e7edf5]'}`} /></div><div className="mt-8 rounded-3xl bg-[#eaf3ff] p-6 text-center sm:p-10"><p className="text-xs uppercase tracking-[0.14em] text-[#93b2d9]">English word</p><p className="mt-5 text-5xl font-black text-[#4f9df1] sm:text-7xl">{active?.en || '-'}</p><p className="mt-3 text-sm text-[#7f96b6]">Words today: {lessonWords.length}/20</p></div>{learnStep === 1 && <><input value={learnInput} onChange={(e) => { setLearnInput(e.target.value); if (checkState !== 'idle') setCheckState('idle') }} placeholder="..." className="mt-6 w-full rounded-3xl border-2 border-[#75b2f8] bg-white px-5 py-4 text-center text-2xl text-[#1e2b3d]" /><button onClick={checkStep1} className={`mt-4 w-full rounded-3xl px-5 py-4 text-2xl font-black text-white sm:text-3xl ${checkState === 'correct' ? 'bg-[#2ea66f]' : checkState === 'wrong' ? 'bg-[#e05252]' : 'bg-[#4f9df1]'}`}>{t.check}</button>{checkState !== 'idle' && active && <div className="mt-4 rounded-2xl bg-white p-5 text-left"><p className="text-xs uppercase tracking-wide text-[#8ea0b8]">Translation</p><p className="mt-1 text-2xl font-bold text-[#4f9df1]">{wordAssist?.translation || (lang === 'ru' ? active.ru : active.uz)}</p><p className="mt-3 text-xs uppercase tracking-wide text-[#8ea0b8]">Description</p><p className="mt-1 text-base text-[#526884]">{wordAssist?.description || (lang === 'ru' ? (active.ru_description || active.ru) : (active.uz_description || active.uz))}</p></div>}</>}{learnStep === 2 && <div className="mt-6 space-y-4"><div className="rounded-2xl bg-white p-5 text-lg text-[#526884]">{sentence || '...'}</div><div className="grid gap-2">{step2Options.map((opt) => <button key={opt} onClick={() => checkStep2(opt)} className="rounded-2xl border border-[#dce7f4] bg-white px-5 py-4 text-left text-xl font-semibold hover:border-[#4f9df1]">{opt}</button>)}</div></div>}{learnStep === 3 && <div className="mt-6 space-y-4"><div className="rounded-2xl bg-white p-5 text-center"><p className="text-sm uppercase tracking-wide text-[#8ea0b8]">Translation</p><p className="mt-2 text-3xl font-black text-[#4f9df1]">{wordAssist?.translation || (active ? (lang === 'ru' ? active.ru : active.uz) : '-')}</p></div><div className="grid gap-3 sm:grid-cols-2"><button onClick={() => checkStep3(1)} className="rounded-2xl bg-[#fff2f2] px-5 py-4 text-xl font-bold text-[#ef5f5f]">Hard (1)</button><button onClick={() => checkStep3(5)} className="rounded-2xl bg-[#edf9f1] px-5 py-4 text-xl font-bold text-[#30a46c]">Easy (5)</button></div><button onClick={completeDay} className="w-full rounded-2xl bg-[#4f9df1] px-5 py-4 text-xl font-black text-white">{t.finish}</button></div>}</div>}
